...
const swaggerOptions = {
  definition: {
    openapi: "3.0.0",
    info: {
      title: "Practice 21 RBAC API with Redis cache",
      version: "1.2.0",
      description: "Backend for React client with JWT auth, refresh tokens, role-based access control and Redis caching",
    },
    components: {
      securitySchemes: {
        bearerAuth: {
...
async function initRedis() {
  try {
    ...
    console.warn("Redis is not available; cache is disabled.");
...
async function invalidateCacheKeys(keys) {
  ...
  if (!redisClient.isReady || filteredKeys.length === 0) {
    return;
  }
...
async function invalidateUsersCache(userId = null) {
  await invalidateCacheKeys(["users:all", userId ? `users:${userId}` : null]);
}

async function invalidateProductsCache(productId = null) {
  await invalidateCacheKeys(["products:all", productId ? `products:${productId}` : null]);
}
...
app.put(
  ...
  async (req, res) => {
    ...
    await invalidateProductsCache(products[productIndex].id);
